 ...
 * DistanceFunction class in 2D. The test wall is defined as a vertical wall at x = 0, spanning y in
 * [-1, 1]. The tests cover points in front of the wall, behind the wall, on the wall surface, and
 * beyond the finite extent of the wall. The expected values and gradients are compared against the
 * computed results with a specified tolerance.
 */
class DistanceFunctionTest2D : public ::testing::Test
{
protected:
  DistanceFunctionTest2D()
    : wall(dealii::Point<2>(0., 0.), dealii::Tensor<1, 2>{{1., 0.}}, std::array<double, 1>{{1.0}})
  {}
  // MeltPoolDG::Functions::DistanceFunction<2> wall;
  MeltPoolDG::Functions::Distance::Plane<2, double> wall;
};

/**
 * Test the value() method of DistanceFunction in 2D for a point in front of the wall. The expected
 * value is the perpendicular distance from the point to the wall, which should be positive.
 */
TEST_F(DistanceFunctionTest2D, Value_InFrontOfWall)
{
...
/**
 * Test the value() method of DistanceFunction in 2D for a point behind the wall. The expected value
 * is the perpendicular distance from the point to the wall, which should be positive.
 */
TEST_F(DistanceFunctionTest2D, Value_BehindWall)
{
...
/**
 * Test the value() method of DistanceFunction in 2D for a point on the wall surface. The expected
 * value is zero, as the point lies exactly on the wall.
 */
TEST_F(DistanceFunctionTest2D, Value_OnWallSurface)
{
...
/**
 * Test the value() method of DistanceFunction in 2D for a point beyond the finite extent of the
 * wall. The expected value is the Euclidean distance from the point to the closest point on the
 * wall, which is not simply the perpendicular offset.
 */
TEST_F(DistanceFunctionTest2D, Value_BeyondFiniteExtent)
{
...
/**
 * Test the gradient() method of DistanceFunction in 2D for a point on the wall surface. The
 * gradient is not well-defined at this point.
 */
TEST_F(DistanceFunctionTest2D, Gradient_OnSurfaceSingularity)
{
  const dealii::Point<2> point_on_surface(0., 0.5);

  // At the singularity (r_norm == 0), DistanceFunction::gradient() falls back to the wall's
  // normal instead of dividing by zero.
  const auto gradient = wall.gradient(point_on_surface);
  EXPECT_NEAR(gradient[0], 1.0, tolerance);
  EXPECT_NEAR(gradient[1], 0.0, tolerance);
}
...
/**
 * The fixture tests below check the correctness of the value() and gradient() methods of the
 * DistanceFunction class in 3D. The test wall lies in the plane x = 0, with half-extents 1.0
 * and 1.5 along its two in-plane tangent directions. The tests cover points in front of the wall,
 * behind the wall, on the wall surface, and beyond the finite extent of the wall along one or both
 ...
 */
class DistanceFunctionTest3D : public ::testing::Test
{
protected:
  DistanceFunctionTest3D()
    : wall(dealii::Point<3>(0., 0., 0.),
           dealii::Tensor<1, 3>{{1., 0., 0.}},
           std::array<double, 2>{{1.0, 1.5}},
           dealii::Tensor<1, 3>{{0., 1., 0.}})
  {}
  // MeltPoolDG::Functions::DistanceFunction<3> wall;
  MeltPoolDG::Functions::Distance::Plane<3, double> wall;
};

/**
 * Test the value() method of DistanceFunction in 3D for a point in front of the wall. The expected
 * value is the perpendicular distance from the point to the wall, which should be positive.
 */
TEST_F(DistanceFunctionTest3D, Value_InFrontOfWall)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point behind the wall. The expected value
 * is the perpendicular distance from the point to the wall, which should be positive.
 */
TEST_F(DistanceFunctionTest3D, Value_BehindWall)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point on the wall surface. The expected
 * value is zero, as the point lies exactly on the wall.
 */
TEST_F(DistanceFunctionTest3D, Value_OnWallSurface)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point on the wall surface that is offset
 * from the center along both tangential directions but still within the finite extents. The
 * expected value is zero, as the point still lies exactly on the wall.
 */
TEST_F(DistanceFunctionTest3D, Value_OnWallSurfaceOffCenter)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point beyond the finite extent of the
 * wall along the first tangential direction only. The expected value is the Euclidean distance from
 * the point to the closest point on the wall, which is not simply the perpendicular offset.
 */
TEST_F(DistanceFunctionTest3D, Value_BeyondFirstExtent)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point beyond the finite extent of the
 * wall along the second tangential direction only. The expected value is the Euclidean distance
 * from the point to the closest point on the wall, which is not simply the perpendicular offset.
 */
TEST_F(DistanceFunctionTest3D, Value_BeyondSecondExtent)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point beyond the finite extent of the
 * wall along both tangential directions. The expected value is the Euclidean distance from the
 * point to the closest point on the wall, which is not simply the perpendicular offset.
 */
TEST_F(DistanceFunctionTest3D, Value_BeyondBothExtents)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point directly above the edge of the wall
 ...
 */
TEST_F(DistanceFunctionTest3D, Value_DirectlyAboveEdge)
{
...
/**
 * Test the value() method of DistanceFunction in 3D for a point that lies in the wall plane but
 * outside the finite extent of the wall. The expected value is the distance from the point to the
 * closest point on the wall along the tangential direction.
 */
TEST_F(DistanceFunctionTest3D, Value_InPlaneButOutside)
{
...
TEST_F(DistanceFunctionTest3D, Gradient_BeyondBothExtents)
{
  ...
  const auto   gradient = wall.gradient(point_beyond);
  const double norm     = std::numbers::sqrt3;
  EXPECT_NEAR(gradient[0], 1.0 / norm, tolerance);
...
/**
 * Test the gradient() method of DistanceFunction in 3D for a point on the wall surface, offset from
 * the center along both tangential directions. The gradient is not well-defined at this point.
 */
TEST_F(DistanceFunctionTest3D, Gradient_OnSurfaceSingularity)
{
